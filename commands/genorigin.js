...
const fs = require("fs");
var filename = "accounts/origin.txt";
...
module.exports.run = async (bot, message, args) => {
	...
	//send 1st line
	if (message.channel.id === config.origin) {
		get_line(filename, 0, function(err, line){ //0 represents first line of file
			...
			data = data.toString();
			var position = data.toString().indexOf('\n');
			if (position != -1) { console.log("good") } else { return message.channel.send("**There are no Origin in stock.**") };
			...
			request({
				...
				.addField("https://iloot.it/?m=" + obj.short, "**Your Origin account.**")
...
console.log(message.guild.id)
		...
		message.channel.send(`Sorry, this is not <#${config.origin}>!`)
	}
}