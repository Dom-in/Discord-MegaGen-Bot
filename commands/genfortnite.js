...
const fs = require("fs");
var filename = "accounts/fortnite.txt";
...
module.exports.run = async (bot, message, args) => {
	...
	function get_line(filename, line_no, callback) {
		...
		if(+line_no > lines.length){
			...
			bot.channels.get(`627571304089780239`).send(`The is no stock in free gen!`)
		}
	...
	//send 1st line
	if (message.channel.id === config.fortnite) {
		get_line(filename, 0, function(err, line){ //0 represents first line of file
			...
			data = data.toString();
			var position = data.toString().indexOf('\n');
			if (position != -1) { console.log("good") } else { return message.channel.send("**There are no Fortnite in stock.**") };
			...
			request({
				...
				.addField("https://iloot.it/?m=" + obj.short, "**Your Fortnite account.**")
...
console.log(message.guild.id)
		...
		message.channel.send(`Sorry, this is not <#${config.fortnite}>!`)
	}
}