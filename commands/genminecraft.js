...
const fs = require("fs");
var filename = "accounts/minecraft.txt";
...
module.exports.run = async (bot, message, args) => {
	...
	//send 1st line
	if (message.channel.id === config.minecraft) {
		get_line(filename, 0, function(err, line){ //0 represents first line of file
			...
			data = data.toString();
			var position = data.toString().indexOf('\n');
			if (position != -1) { console.log("good") } else { return message.channel.send("**There are no Minecraft in stock.**") };
			...
			request({
				...
				.addField("https://iloot.it/?m=" + obj.short, "**Your Minecraft account.**")
...
console.log(message.guild.id)
		...
		message.channel.send(`Sorry, this is not <#${config.minecraft}>!`)
	}
}